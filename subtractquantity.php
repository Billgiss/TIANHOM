...
$q69 = "SELECT quantity FROM cart_details WHERE cart_id = $cart_id AND product_id = $productid";
$result69 = mysqli_query($mysqli,$q69);
while($row=$result69->fetch_array()){
    $quantity1 = intval($row['quantity']);
    echo $quantity1;
}
    if($quantity1 == 0){
        $q0 = "DELETE FROM cart_details WHERE cart_id = $cart_id AND product_id = $productid";
        mysqli_query($mysqli,$q0);
    }
